...
class ScientificCalculatorController extends GetxController {
  ...
  getResult(double? leftOperand, double? rightOperand, String operator) {
    ...
    if (operator == AppStrings.buttoNtangent && rightOperand == 90) {
      return result.value = "Infinity";
    } else {
      result.value = Calculator(calculatorType)
          .doCalculate(leftOperand!, rightOperand!, operator)
          .toString();
    }
  }
  ...
  void buttonPressed(String buttonText) {
    if (buttonText == AppStrings.buttonCancelAll) {
      ...
      if (input.value != AppStrings.buttonDecimal &&
          input.value != "" &&
          keyPressed == true &&
          operatorPressed == false) {
        if ((!input.value.contains(AppStrings.buttonAddition)) &&
      ...
      if (operator == AppStrings.buttoNsquare ||
          operator == AppStrings.buttonCube) {
    ...
    } else if (buttonText == AppStrings.buttonOne ||
